    ...
    for (int i = 0; i < numberOfPlayers; i++)
    {
        for (int j = 0; j < numberOfGamePiecesPerPlayer; j++)
        {
            if (gamePieces[i][j]->getPosition() == 0) {
                ...
                if (x == 16 && y == 0) { doubleBlue++; }
                if (x == 24 && y == 0) { doubleYellow++; }
                if (x == 36 && y == 12) { doubleGreen++; }
                if (x == 20 && y == 20) { doubleMagenta++; }
                if (x == 12 && y == 20) { doubleCyan++; }

                if (doubleRed > 1) { visualBoard[7][0] = color_red + "! " + color_reset; }
                if (doubleBlue > 1) { visualBoard[0][17] = color_red + "! " + color_reset; }
                if (doubleYellow > 1) { visualBoard[0][24] = color_red + "! " + color_reset; }
                if (doubleGreen > 1) { visualBoard[13][36] = color_red + " !" + color_reset; }
                if (doubleMagenta > 1) { visualBoard[20][19] = color_red + " !" + color_reset; }
                if (doubleCyan > 1) { visualBoard[20][11] = color_red + " !" + color_reset; }
